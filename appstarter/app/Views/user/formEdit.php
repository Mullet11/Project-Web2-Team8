<!-- Overlay Modal Container for Edit -->
<div id="modalFormEdit" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    <!-- Darkened Background Backdrop -->
    <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onclick="closeFormEdit()"></div>
    ...
    <div class="relative w-full max-w-[900px] bg-white rounded-[20px] shadow-2xl p-8 border border-slate-100 overflow-y-auto max-h-[90vh] animate-in fade-in zoom-in duration-200">
        ...
        <!-- Title -->
        <h3 class="text-2xl font-bold text-[#FFB800] text-center mb-6">Form Edit Booking</h3>

        <!-- Form (Grid of 2 Columns) -->
        <form action="<?= base_url('booking/update') ?>" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            <?= csrf_field() ?>
            <input type="hidden" name="id_booking" id="edit_id_booking" value="">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="edit_kategori_peminjaman" name="kategori_peminjaman" onchange="handleEditKategoriChange(this.value)" required
                            ...
                            <option value="kelas_pembelajaran">Kelas Pembelajaran</option>
                ...
                <div>
                    ...
                    <input type="text" id="edit_nama" name="nama" placeholder="Masukkan Nama" required
                ...
                <div>
                    ...
                    <input type="text" id="edit_nim" name="nim" placeholder="Masukkan NIM" required
                           class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-[10px] text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#FFB800] focus:ring-1 focus:ring-[#FFB800] transition-all text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="tel" id="edit_whatsapp" name="whatsapp" placeholder="Masukkan No. WhatsApp Aktif" required
                           class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-[10px] text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#FFB800] focus:ring-1 focus:ring-[#FFB800] transition-all text-sm">
                </div>
            </div>
            ...
            <div class="space-y-4 flex flex-col justify-between">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="edit_perihal" name="perihal" placeholder="Masukkan Perihal Peminjaman" required
                               class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-[10px] text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#FFB800] focus:ring-1 focus:ring-[#FFB800] transition-all text-sm">
                    </div>

                    <!-- Conditional Section for Kelas Pembelajaran & Kelas Pengganti -->
                    <div id="edit_field_group_kelas" class="space-y-4">
                        <!-- Dosen Field -->
                        <div>
                            <label class="block text-slate-900 font-bold text-[14px] mb-2">Dosen</label>
                            <div class="relative">
                                <select id="edit_select_dosen" name="dosen" required
                                        class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-[10px] text-slate-800 focus:outline-none focus:border-[#FFB800] focus:ring-1 focus:ring-[#FFB800] transition-all text-sm appearance-none cursor-pointer">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select id="edit_select_matkul" name="matkul" required
                                        class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-[10px] text-slate-800 focus:outline-none focus:border-[#FFB800] focus:ring-1 focus:ring-[#FFB800] transition-all text-sm appearance-none cursor-pointer">
                    ...
                    <div id="edit_field_group_kegiatan" class="space-y-4 hidden">
                        <!-- Nama Kegiatan Field -->
                        <div>
                            <label class="block text-slate-900 font-bold text-[14px] mb-2">Nama Kegiatan</label>
                            <input type="text" id="edit_input_nama_kegiatan" name="nama_kegiatan" placeholder="Masukkan Nama Kegiatan"
                                   class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-[10px] text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#FFB800] focus:ring-1 focus:ring-[#FFB800] transition-all text-sm">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="time" id="edit_waktu_mulai" name="waktu_mulai" readonly required
                        ...
                        <div>
                            ...
                            <input type="time" id="edit_waktu_selesai" name="waktu_selesai" readonly required
                                   class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-[10px] text-slate-500 cursor-not-allowed focus:outline-none text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-4">
                    <button type="submit" 
                            class="w-full py-3.5 bg-[#FFB800] hover:bg-[#e0a400] text-white font-bold text-[15px] rounded-[10px] transition-all active:scale-[0.99] shadow-sm">
                        Simpan Perubahan
                    </button>
                </div>
            </div>
...
<!-- Category display toggler javascript for Edit -->
<script>
    function handleEditKategoriChange(val) {
        const fieldKelas = document.getElementById('edit_field_group_kelas');
        const fieldKegiatan = document.getElementById('edit_field_group_kegiatan');
        
        const inputNamaKegiatan = document.getElementById('edit_input_nama_kegiatan');
        const selectDosen = document.getElementById('edit_select_dosen');
        const selectMatkul = document.getElementById('edit_select_matkul');
    ...
    function submitFormEdit(e) {
        e.preventDefault();
        alert("Perubahan booking berhasil disimpan!");
        closeFormEdit();
    }
</script>
